...
import { suppliersAPI, chartOfAccountsAPI, citiesAPI } from '../services/api';
...
export const Suppliers = () => {
  ...
  const createSupplierMutation = useMutation(suppliersAPI.createSupplier, {
    onSuccess: () => {
  ...
  const deleteSupplierMutation = useMutation(suppliersAPI.deleteSupplier, {
    onSuccess: () => {
      queryClient.invalidateQueries('suppliers');
  ...
  const allSuppliers = suppliers?.data?.suppliers || [];
  ...
  const handleSave = (formData) => {
    ...
    if (selectedSupplier) {
      ...
      createSupplierMutation.mutate(cleanData);
    }
  };
