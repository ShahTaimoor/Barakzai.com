...
const fetchCashPayments = async (params = {}) => {
  ...
  const response = await fetch(`/api/cash-payments?${queryParams}`, {
    headers: {
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch cash payments');
  }

  return response.json();
};

const createCashPayment = async (data) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(error.message || 'Failed to create cash payment');
  }
...
const CashPayments = () => {
  ...
  const { 
    ...
    ['cashPayments', { ...filters, ...pagination, sortConfig }],
    () => fetchCashPayments({ ...filters, ...pagination, sortConfig }),
    {
  ...
  // Mutations
  const createMutation = useMutation(createCashPayment, {
    onSuccess: () => {
  ...
  const paginationInfo = cashPaymentsData?.data?.pagination || {};
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="text-gray-600">Manage and view all cash payment transactions</p>
        </div>
...
{(() => {
          ...
          <div className="flex justify-end space-x-3 mt-6 pt-4 border-t border-gray-200">
            ...
            >
              ...
              <span>{createMutation.isLoading ? 'Saving...' : 'Save Payment'}</span>
            </button>
          </div>
        </div>
      </div>
