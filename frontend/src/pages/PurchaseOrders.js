...
export const PurchaseOrders = ({ tabId }) => {
  ...
  const { 
    ...
    ['purchaseOrders', { ...filters, ...pagination, sortConfig }],
    () => {
      // Filter out empty values to avoid API errors
      const cleanFilters = Object.fromEntries(
        Object.entries(filters).filter(([_, value]) => value !== '' && value != null)
      );
      return purchaseOrdersAPI.getPurchaseOrders({ ...cleanFilters, ...pagination, sortConfig });
    },
    {
      keepPreviousData: true,
      onError: (error) => {
        console.error('Purchase Orders API Error:', error.response?.data || error.message);
  ...
  // Mutations
  const createMutation = useMutation(purchaseOrdersAPI.createPurchaseOrder, {
    onSuccess: () => {
  ...
  const updateMutation = useMutation(
    ({ id, data }) => purchaseOrdersAPI.updatePurchaseOrder(id, data),
    {
      onSuccess: () => {
        queryClient.invalidateQueries('purchaseOrders');
      ...
      onError: (error) => {
        ...
        const errorMessage = error.response?.data?.message || error.message || 'Failed to update purchase order';
        toast.error(errorMessage);
      }
    }
  );

  const deleteMutation = useMutation(purchaseOrdersAPI.deletePurchaseOrder, {
    onSuccess: () => {
      queryClient.invalidateQueries('purchaseOrders');
  ...
  const confirmMutation = useMutation(purchaseOrdersAPI.confirmPurchaseOrder, {
    onSuccess: () => {
      queryClient.invalidateQueries('purchaseOrders');
  ...
  const cancelMutation = useMutation(purchaseOrdersAPI.cancelPurchaseOrder, {
    onSuccess: () => {
      queryClient.invalidateQueries('purchaseOrders');
