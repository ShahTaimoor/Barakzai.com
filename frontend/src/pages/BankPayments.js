...
import { suppliersAPI, customersAPI, banksAPI, chartOfAccountsAPI } from '../services/api';
import PrintModal from '../components/PrintModal';

// API functions
const fetchBankPayments = async (params = {}) => {
  ...
  const response = await fetch(`/api/bank-payments?${queryParams}`, {
    headers: {
...
const createBankPayment = async (data) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(error.message || 'Failed to create bank payment');
  }
...
const BankPayments = () => {
  ...
  const { 
    ...
    ['bankPayments', { ...filters, ...pagination, sortConfig }],
    () => fetchBankPayments({ ...filters, ...pagination, sortConfig }),
    {
  ...
  // Mutations
  const createMutation = useMutation(createBankPayment, {
    onSuccess: () => {
  ...
  const paginationInfo = bankPaymentsData?.data?.pagination || {};
