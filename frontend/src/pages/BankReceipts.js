...
const fetchBankReceipts = async (params = {}) => {
  ...
  const response = await fetch(`/api/bank-receipts?${queryParams}`, {
    headers: {
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch bank receipts');
  }

  return response.json();
};

const createBankReceipt = async (data) => {
  const response = await fetch('/api/bank-receipts', {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(error.message || 'Failed to create bank receipt');
  }
...
const BankReceipts = () => {
  ...
  const { 
    ...
    ['bankReceipts', { ...filters, ...pagination, sortConfig }],
    () => fetchBankReceipts({ ...filters, ...pagination, sortConfig }),
    {
  ...
  // Mutations
  const createMutation = useMutation(createBankReceipt, {
    onSuccess: () => {
      queryClient.invalidateQueries('bankReceipts');
  ...
  const paginationInfo = bankReceiptsData?.data?.pagination || {};
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="text-gray-600">Manage and view all bank receipt transactions</p>
        </div>
