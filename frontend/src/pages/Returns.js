...
const Returns = () => {
  ...
  const { 
    ...
    ['return-stats', filters.startDate, filters.endDate],
  ...
  const updateStatusMutation = useMutation(
    ({ returnId, status, notes }) => 
      returnsAPI.updateReturnStatus(returnId, { status, notes }),
    {
      onSuccess: (data, variables) => {
        showSuccessToast(`Return status updated to ${variables.status}`);
        queryClient.invalidateQueries(['returns']);
        queryClient.invalidateQueries(['return-stats']);
  ...
  const addNoteMutation = useMutation(
    ({ returnId, note, isInternal }) => 
      returnsAPI.addNote(returnId, { note, isInternal }),
    {
      onSuccess: () => {
        showSuccessToast('Note added successfully');
        queryClient.invalidateQueries(['returns']);
        if (selectedReturn) {
          queryClient.invalidateQueries(['return', selectedReturn._id]);
        }
      },
      onError: (error) => {
  ...
  const deleteReturnMutation = useMutation(
    (returnId) => returnsAPI.deleteReturn(returnId),
    {
      onSuccess: () => {
        showSuccessToast('Return deleted successfully');
        queryClient.invalidateQueries(['returns']);
        queryClient.invalidateQueries(['return-stats']);
        queryClient.invalidateQueries(['return-trends']);
      },
      onError: (error) => {
  ...
  const addCommunicationMutation = useMutation(
    ({ returnId, type, message, recipient }) => 
      returnsAPI.addCommunication(returnId, { type, message, recipient }),
    {
      onSuccess: () => {
  ...
  const handleReturnSelect = async (returnId) => {
    try {
      const response = await returnsAPI.getReturn(returnId);
  ...
  const handleAddNote = (note, isInternal = false) => {
    if (!selectedReturn) return;
  ...
  const handleAddCommunication = (type, message, recipient = null) => {
    if (!selectedReturn) return;
    ...
    <ResponsiveContainer className="space-y-6">
      ...
      {showDetailModal && selectedReturn && (
        <ReturnDetailModal
          ...
          isLoading={updateStatusMutation.isLoading || addNoteMutation.isLoading || addCommunicationMutation.isLoading}
        />
      )}
    </ResponsiveContainer>
  );
