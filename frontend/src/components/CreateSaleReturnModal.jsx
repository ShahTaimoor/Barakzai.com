...
const CreateSaleReturnModal = ({ isOpen, onClose, onSuccess, sale, customer }) => {
  ...
  const handleAddItem = (orderItem) => {
    ...
    const itemPrice = orderItem.price || orderItem.unitPrice || orderItem.unitCost || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!sale) {
      showErrorToast('Sale information is missing');
      return;
    }
                    ...
                    <div className="border rounded-lg p-4">
                      ...
                      <div className="space-y-4">
                        {formData.items.map((item, index) => {
                          const orderItem = saleItems.find(oi => oi._id === item.originalOrderItem);
